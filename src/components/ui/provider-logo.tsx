...
export function ProviderLogo({ id, size = 22, title }: { id: string; size?: number; title?: string }) {
...
export function ProviderRow({ ids, size = 22, max = 4, label }: { ids: string[]; size?: number; max?: number; label?: string }) {
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      ...
      {visible.map(id => <ProviderLogo key={id} id={id} size={size} />)}
